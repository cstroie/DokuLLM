...
(function() {
    ...
    function replaceTextareaWithDiv() {
        ...
        // Convert textarea content to HTML with proper paragraph structure
        const textContent = textarea.value;
        div.innerHTML = textToHtml(textContent);
        ...
        // Sync content on any input change
        div.addEventListener('input', function() {
            console.log('DokuLLM: Syncing content to hidden textarea on input');
            textarea.value = htmlToText(div);
        });
    ...
    function processText(action) {
        ...
        hiddenTextarea.value = htmlToText(editor);
        ...
        .then(data => {
            ...
            if (selectedText) {
                ...
                editor.innerHTML = textToHtml(metadataContent + data.result);
            }
        })
        .catch(error => {
    ...
    function processCustomPrompt(customPrompt) {
        ...
        const hiddenTextarea = document.getElementById('wiki__text');
        if (!editor || !hiddenTextarea) {
            console.log('DokuLLM: Editor or hidden textarea not found for custom prompt');
            return;
        }
        ...
        if (editor.tagName === 'TEXTAREA') {
            ...
            if (selection.rangeCount > 0) {
                ...
                if (range.startOffset === 0 && range.endOffset === text.length) {
                    const metadata = extractMetadata(text);
                    const fullTextHtml = textToHtml(metadata + newText);
                    editor.innerHTML = fullTextHtml;
                } else {
                    range.deleteContents();
                
                    // Convert text to HTML with proper paragraph structure
                    const newTextHtml = textToHtml(newText);
                    const tempDiv = document.createElement('div');
                    tempDiv.innerHTML = newTextHtml;
                
                    // Check if we're inside a paragraph and need to split it
                    let container = range.startContainer;
                    let parentParagraph = null;
                    
                    // Find the closest paragraph parent
                    while (container && container !== editor) {
                        if (container.nodeType === Node.ELEMENT_NODE && container.tagName === 'P') {
                            parentParagraph = container;
                            break;
                        }
                    ...
                    if (parentParagraph) {
                        // We're inside a paragraph, need to split it
                        const afterParagraph = parentParagraph.cloneNode(false); // Clone without children
                        afterParagraph.innerHTML = '';
                        
                        // Extract content after the insertion point
                        const rangeToEnd = document.createRange();
                        rangeToEnd.setStart(range.endContainer, range.endOffset);
                        rangeToEnd.setEndAfter(parentParagraph);
                        const afterContent = rangeToEnd.extractContents();
                        afterParagraph.appendChild(afterContent);
                        
                        // Insert the new content and the after content
                        ...
                        parentParagraph.parentNode.insertBefore(fragment, parentParagraph.nextSibling);
                        
                        // If the afterParagraph is empty, remove it
                        if (afterParagraph.innerHTML.trim() === '') {
                            afterParagraph.parentNode.removeChild(afterParagraph);
                        }
                    } else {
                        // Not inside a paragraph, insert normally
                        const fragment = document.createDocumentFragment();
                        while (tempDiv.firstChild) {
                            fragment.appendChild(tempDiv.firstChild);
                        }
                ...
                // No selection, replace entire content with proper HTML formatting
                const newTextHtml = textToHtml(newText);
                editor.innerHTML = newTextHtml;
            }
        }
    }
    ...
    function insertTemplateContent(templateId) {
        ...
        const hiddenTextarea = document.getElementById('wiki__text');
        if (!editor || !hiddenTextarea) {
            console.log('DokuLLM: Editor or hidden textarea not found for template insertion');
            return;
        }
        
        // Check if editor has focus or contains the current selection
        const selection = window.getSelection();
        let isEditorFocused = false;
        
        if (selection.rangeCount > 0) {
            const range = selection.getRangeAt(0);
            isEditorFocused = editor.contains(range.startContainer);
        }
        ...
        .then(data => {
            ...
            if (selection.rangeCount > 0 && editor.contains(selection.getRangeAt(0).startContainer)) {
                ...
                const templateHtml = textToHtml(data.result.content);
                const tempDiv = document.createElement('div');
                ...
                // Insert template content at the correct level
                const fragment = document.createDocumentFragment();
                while (tempDiv.firstChild) {
                    fragment.appendChild(tempDiv.firstChild);
                }
                ...
                // No valid selection in editor, insert at the beginning
                const templateHtml = textToHtml(data.result.content);
                const currentContent = editor.innerHTML;
                editor.innerHTML = templateHtml + currentContent;
    ...
    function textToHtml(text) {
        ...
        // Split text into paragraphs (one or more blank lines)
        const paragraphs = text.split(/\n\s*\n/);
        
        // Filter out empty paragraphs and convert each to HTML
    ...
    /**
     * Convert HTML to text preserving newlines
     * 
     * @param {HTMLElement} element - The contenteditable element
     * @returns {string} Text representation of the HTML
     */
    function htmlToText(element) {
        ...
        // Handle <br> tags that are not inside p or div
        const brs = clone.querySelectorAll('br');
        brs.forEach(br => {
            br.parentNode.replaceChild(document.createTextNode('\n'), br);
        });
        
        // Get text content and clean up extra newlines
        let text = clone.textContent;
        
        // Remove leading/trailing whitespace but preserve paragraph structure
        text = text.replace(/^\s+|\s+$/g, '');
        
        // Normalize multiple newlines
        text = text.replace(/\n{3,}/g, '\n\n'); // More than 2 newlines become 2
        text = text.replace(/\n{2,}/g, '\n\n'); // Exactly 2 newlines stay as paragraphs
