...
A DokuWiki plugin that integrates Large Language Models (LLM) to enhance content creation and editing capabilities with semantic search through ChromaDB integration.
...
The plugin integrates with ChromaDB for intelligent document management:
- **Document Processing**: Smart parsing of DokuWiki files with metadata extraction
- **Vector Storage**: Documents stored as embeddings for semantic search
- **Update Optimization**: Timestamp-based checking to avoid reprocessing unchanged files
- **Direct Retrieval**: Access documents by ID with rich metadata
...
- DokuWiki installation
- Access to an LLM API (e.g., OpenAI, Ollama, or any compatible service)
- ChromaDB server for semantic search capabilities
- PHP with cURL support
...
- **Top-k**: Top-k sampling parameter (integer >= 1)
- **Min-p**: Minimum probability threshold (0.0-1.0)
- **Think**: Enable deeper thinking processes in LLM responses
- **Tool Usage**: Allow LLMs to call tools for additional context
